...
const actions: ActionTree<RootState, RootState> = {
  ...
  setProvider (_, { provider, connector }) {
    const contractData = [
      WaffleMakerJson,
      {
        ...
        networks: {
          [this.$config.networkChainId]: {
            address: this.$config.currencyAddress
          }
        }
      }
    ]

    const networkChainId = this.$config.networkChainId.toString()
    if (provider instanceof Harmony) {
      this.$library = provider.blockchain
      contractData.forEach((data) => {
        if (data.networks[networkChainId]) {
          const contract = provider.contracts.createContract(
            data.abi,
            data.networks[networkChainId].address
          )
          if (connector) {
      ...
      contractData.forEach((data) => {
        if (data.networks[networkChainId]) {
          // @ts-ignore
          this.$contracts[data.contractName] = new web3.eth.Contract(
            data.abi,
            data.networks[networkChainId].address
          )
        }
  ...
  async dispatchTransaction (
    { dispatch, getters },
    { title, transaction, successCallback, oneCost, currencyCost }
  ) {
  ...
  async spendCurrency ({ dispatch }, { amount, action }) {
    ...
    const waffleMakerAddress =
      this.$contracts.WaffleMaker._address ||
      this.$contracts.WaffleMaker.address
    if (currencyToken.compareBalance(amount) === -1) {
      ...
      await dispatch('dispatchTransaction', {
        ...
        transaction: currencyTokenData.getApproveTransaction(
          this,
          waffleMakerAddress,
          currencyToken.balance
        ),
